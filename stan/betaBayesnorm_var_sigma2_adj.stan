// betaD-Bayesian Gaussian Likelihood Model (multiplying the scale parameter sigma2 by a constant sigma2_adj)
...
model {
   ...
   for(i in 1:n){
     target += w*((1/beta)*exp(normal_lpdf(y[i,1] | mu, sqrt(sigma2_adj*sigma2)))^(beta) - int_term);
   }
}
