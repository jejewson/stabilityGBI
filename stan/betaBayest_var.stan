...
transformed parameters
{
  ...
  int_term = (tgamma((df + 1.0)/2.0)^(beta + 1.0)*tgamma((beta*df + beta + df)/2.0))/
             ((1.0 + beta)*tgamma(df/2)^(beta + 1)*tgamma((beta*df + beta + df + 1.0)/2.0)*(df)^((beta)/2.0)*pi()^((beta)/2.0)*sigma2^(beta/2.0));
...
model {
   ...
   for(i in 1:n){
    target += w*((1.0/beta)*exp(student_t_lpdf(y[i,1] | df, mu, sqrt(sigma2)))^(beta) - int_term);
  }
}
